pipeline {
    ...
    stages {
            ...
            steps {
                ...
                script {
                    if ( env.BRANCH_NAME == 'master' ) {
                        containerVersion = getVersionFromContainer("liatrio/ldop-ldap:${env.BRANCH_NAME}")
                        failIfVersionExists("liatrio","ldop-ldap",containerVersion)
                        sh "docker build -t liatrio/ldop-ldap:${containerVersion} ."
                    }
                }
            }
            post {
        ...
        stage('Push to dockerhub') {
            ...
            steps {
                script { CHANGED = "NO" }
                sh "docker tag liatrio/ldop-ldap:${env.BRANCH_NAME} liatrio/ldop-ldap:latest"
                sh "docker push liatrio/ldop-ldap:latest"
                script {
                    if ( env.BRANCH_NAME == 'master' )
                        sh "docker push liatrio/ldop-ldap:${containerVersion}"
                }
            }
            post {
                success { script { STATUS = "SUCCESS" } }
